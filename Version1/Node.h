...
class SignNode : public Node {
	public: 
		...
		SignNode(Node *arg){
			...
			std::cout << "Using Sign Node \n";
			/* Evaluation */
			myResult = -arg->getResult();
		}
...
class TimesNode : public Node {
        public:
                /* @Brief Constructor for two number objects */
...
/* @Brief Multiplication class for double * number object */
class TimesNode2 : public Node {
        public:
                /* @Brief Constructor for two number objects */
                TimesNode2(double lhs, Node* rhs) {
                        myArguments.resize(1);
                        rhs->variable = lhs;
			myArguments[0] = rhs;
			std::cout << "Using TimesNode2 \n";
                        /* Evaluaton */
                        myResult = lhs * rhs->getResult();
                }

                void propagateAdjoint() override {
                        myArguments[0]->getAdjoint() += myAdjoint*variable;
                }
};
